...
export class FormService {
  ...
  static getNextControl(
    parentControl: AbstractControl | AbstractControl[] | undefined,
    key: string | number
  ...
  static getControl(
    ...
    arrayKeyList?: Array<number>
  ): AbstractControl | AbstractControl[] | undefined {
    //  寻找的控制器
    let control: AbstractControl | AbstractControl[] | undefined = formGroup;

    try {
      //  arrayKeyList的位移下标
      let i: number = 0;
      for (const key of keyList) {
        if (control instanceof FormArray && arrayKeyList && arrayKeyList.length) {
   ...
   * @returns {any}
   */
  static setControl(formGroup: FormGroup, setCntrol: AbstractControl, keyList: Array<string>, arrayKeyList?: Array<number>): void {
    try {
      //  arrayKeyList的位移下标
      let i: number = 0;
      //  setCntrol对应的key
      ...
      keyList.pop();
      // 父级控制器
      let control: AbstractControl | AbstractControl[] | undefined = this.getControl(formGroup, keyList, arrayKeyList);

      if (control instanceof FormGroup) {
        control.setControl(setKey, setCntrol);
      } else if (control instanceof FormArray && arrayKeyList && arrayKeyList.length) {
        control.setControl(arrayKeyList[i++], setCntrol);
      } else {
        throw '根据keyList, arrayKeyList无法设置control';
      }
    } catch (e) {
      console.error('找不到指定control：', e, formGroup, setCntrol, keyList, arrayKeyList);
    }
  }
  /**
   * 表单的正则表达式校验
   ...
   * @returns {FormGroup}
   */
  static toFormGroup(...formControlLists: Array<Array<FormControlBase<any> | DynamicFormGroup>>): FormGroup {
    let group: any = {};
    for (let formControlList of formControlLists) {
      for (let formControlOrDynamicFormGroup of formControlList) {
        ...
        let formControlBaseCheck = function(formControl: FormControlBase<any>): FormControl {
          ...
          // && (formControl.minlength || formControl.minlength === 0) && formControl.maxlength
          if (
            (formControl instanceof Textbox || formControl instanceof TextGroup) &&
            UtilityService.isEmpty(formControl.minlength) &&
            UtilityService.isEmpty(formControl.maxlength)
          ) {
            //  如果是输入数字则校验最大最小
   ...
   * @returns [] 选中的数据
   */
  static getMultiSelected(inputModel: any): Array<any> {
    let selected = [];
