...
@Component({
  selector: 'jhi-field-output-dialog',
...
export class FieldOutputDialogComponent implements OnInit {
  ...
  constructor(private activeModal: NgbActiveModal, private fieldService: FieldService) {}
  ...
  updateLogsData(): void {
    if (this.applyJdlId === undefined) {
      ...
      this.fieldService.getApplyJdlLogs(this.applyJdlId).subscribe(
        (data: string) => {
          this.logs += data;
